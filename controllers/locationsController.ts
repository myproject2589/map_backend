...
async function getAllLocations(req, res) {
  try {
    ...
    const locationQuery = {};
    const allLocations = await LocationModel.aggregate([
      { $match: locationQuery },
    ]);

    const shapeData = await locationsDao.find({
      shape: { $in: ["circle", "marker"] },
    });
    ...
    if (allLocations.length > 0) {
      ...
      res
        .status(400)
        .json({ message: "Failed to fetch locations", data: allLocations });
    }
  } catch (error) {
  ...
  try {
    ...
    if (req.body.shape === "circle") {
      ...
      const locations = await Promise.all(
        locationDocs.map((doc) => locationsDao.insertOne(doc))
      );
...
async function deleteLocation(req, res) {
  try {
    ...
    let deleteLocationDetails = await locationsDao.deleteOne({
      _id: req.body._id,
    });
...
async function deleteShape(req, res) {
  try {
    ...
    if (predeleteLocationDetails.isGroup && predeleteLocationDetails.UniqueId) {
      let deleteLocationDetails = await Shape.deleteMany({
        UniqueId: predeleteLocationDetails.UniqueId,
      });
      logger.debug("Deleted location Details...", deleteLocationDetails);
      res.status(200).json({
        message: "Location deleted successfully.",
...
async function updateShape(req, res) {
  ...
  try {
    ...
    if (shape.type == "rectangle") {
      updatedLocation = await updateLocationById(shape._id, {
        coordinates: shape?.coordinates,
      });
    }else if (shape.type == "circle") {
        updatedLocation = await updateLocationById(shape._id, {
          coordinates: shape?.coordinates,center:shape?.center,radius:shape?.radius
        });
      }
